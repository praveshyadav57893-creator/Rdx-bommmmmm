...
MAIN_OWNER = 8485798078
SUPER_ADMIN_NAME = "@itzrdxking"
SUPER_ADMIN_LINK = "https://t.me/itzrdxking"
SUPER_ADMINS = [8485798078]

BOT_TOKEN = "8817761895:AAFHS9gVwpWLGHwuM4N7PhkSp2EK4HCcGt4"
LOG_CHANNEL_ID = -1004392151338

_DATA_FILE = "Rᴅx_data.json"
_VERSION = "Rᴅxv3.2"
...
def owner_panel_text(d: dict) -> str:
    ...
    return (
        f"{em(EMOJI_CROWN, '👑')} <b>{sc('owner panel')}</b> — Rᴅx Sᴍs Bᴏᴛ {_VERSION}\n"
...
def user_home_text(uid: int, d: dict) -> str:
    ...
    scan_info = get_scan_status()
    return (
        f"{em(EMOJI_PHONE, '📱')} <b>Rᴅx Sᴍs Bᴏᴛ {_VERSION}</b>\n"
        f"<b>Owner:</b> {SUPER_ADMIN_NAME}\n\n"
...
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    chan_log = (
        f"🚀 <b>Rᴅx SMS BLAST ACTIVITY LOG</b>\n\n"
        f"👤 <b>User:</b> {u_name}\n"
        f"🆔 <b>User ID:</b> <code>{uid}</code>\n"
...
async def user_info(cq: CallbackQuery, state: FSMContext):
    await cq.message.edit_text(
        f"{em(EMOJI_GEAR, 'ℹ️')} <b>Rᴅx Sᴍs Bᴏᴛ {_VERSION}</b>\n\n"
...
async def main():
    ...
    dp.include_router(R)
    me = await bot.get_me()
    log.info(f"@{me.username} — Rdx Sᴍs Bᴏᴛ {_VERSION} started!")
    ...
    try:
        await bot.send_message(
            MAIN_OWNER,
            f"{em(EMOJI_ROCKET, '🚀')} <b>Rdx Sᴍs Bᴏᴛ  {_VERSION} Online!</b>\n@{me.username}\n"
